...
##                Total Requests: 1
    ...
    my $msg_q = `sudo /sbin/exim -bpc | xargs echo -n`;
...
sub process_arguments(){
	...
	if (defined $opt_M) {
		if ($opt_M =~ /^(sendmail|qmail|postfix|exim|nullmailer|opensmtpd|cpanelexim)$/) {
			$mailq = $opt_M ;
			print $mailq . "\n";
		}elsif( $opt_M eq ''){
			$mailq = 'sendmail';
...
sub print_help () {
	...
	print "-M (--mailserver) = [ sendmail | qmail | postfix | exim | nullmailer | opensmtpd | cpanelexim ] (default = autodetect)\n";
	print "-h (--help)\n";
	print "-V (--version)\n";
